...
import static uk.gov.justice.laa.crime.enums.HardshipReviewDetailType.ACTION;
...
class HardshipDetailMapperTest {
    ...
    void givenApiHardshipDetailListWithExpenditure_whenToDtoIsInvoked_thenDtoIsMapped() {
        ...
        softly.assertThat(expenditure.getAccepted()).isTrue();
        softly.assertThat(expenditure.getAmount()).isEqualTo(detail.getAmount());
        softly.assertThat(expenditure.getFrequency()).isEqualTo(detail.getFrequency());
        softly.assertThat(expenditure.getReasonCode()).isEqualTo(detail.getDetailReason());
        softly.assertThat(expenditure.getDescription()).isEqualTo(detail.getOtherDescription());
        softly.assertThat(expenditure.getItemCode())
                .isEqualTo(ExtraExpenditureDetailCode.valueOf(
                        detail.getDetailCode().getCode()));
    }

    @Test
    void givenApiHardshipDetailListWithDeniedIncome_whenToDtoIsInvoked_thenDtoIsMapped() {

        List<ApiHardshipDetail> hardshipDetails = TestModelDataBuilder.getApiHardshipReviewDetails(INCOME);
        ...
        softly.assertThat(income.getAccepted()).isFalse();
        softly.assertThat(income.getAmount()).isEqualTo(detail.getAmount());
        softly.assertThat(income.getFrequency()).isEqualTo(detail.getFrequency());
        softly.assertThat(income.getDescription()).isEqualTo(detail.getOtherDescription());
    ...
    void givenApiHardshipDetailListWithSolicitorCosts_whenToDtoIsInvoked_thenDtoIsMapped() {
        ...
        List<ApiHardshipDetail> hardshipDetails = TestModelDataBuilder.getApiHardshipReviewDetails(SOL_COSTS);
        ...
        softly.assertThat(solicitorCosts.getEstimatedTotal()).isEqualTo(detail.getAmount());
    }

    @Test
    void givenApiHardshipDetailListInvalidDetailType_whenToDtoIsInvoked_thenExceptionIsThrown() {
        ApiHardshipDetail detail = new ApiHardshipDetail().withDetailType(ACTION);

        assertThatThrownBy(() -> mapper.toDto(List.of(detail), new HardshipReview()))
