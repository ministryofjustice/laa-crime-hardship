...
class HardshipIntegrationTest {
    ...
    @Test
    void givenAEmptyOAuthToken_whenCalculateHardshipForDetailIsInvoked_thenFailsWithUnauthorizedAccess()
            throws Exception {
        mvc.perform(MockMvcRequestBuilders.post(ENDPOINT_URL_CALCULATE_HARDSHIP)
                        .content("{}")
                        .contentType(APPLICATION_JSON))
    ...
    void givenUnknownHardshipReviewId_whenFindIsInvoked_thenFailsWithBadRequest() throws Exception {
        wiremock.stubFor(get(urlEqualTo("/hardship/" + HARDSHIP_ID)).willReturn(WireMock.badRequest()));

        mvc.perform(MockMvcRequestBuilders.get(ENDPOINT_URL_GET).header(HttpHeaders.AUTHORIZATION, BEARER_TOKEN))
                .andExpect(status().isBadRequest())
    ...
    void givenValidRequest_whenUpdateHardshipIsInvoked_thenOkResponse() throws Exception {
        ...
        wiremock.stubFor(get(urlEqualTo(ENDPOINT_URL_FULL_ASSESSMENT_THRESHOLD
                        + request.getHardship().getReviewDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(BigDecimal.TEN))));

        wiremock.stubFor(put(urlEqualTo("/hardship"))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(response))));

        wiremock.stubFor(get(urlEqualTo("/financial-assessments/"
                        + request.getHardshipMetadata().getFinancialAssessmentId()))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(TestModelDataBuilder.getFinancialAssessmentDTO()))));

        wiremock.stubFor(
                get(urlEqualTo("/hardship/" + request.getHardshipMetadata().getHardshipReviewId()))
                        .willReturn(WireMock.ok()
                                .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                                .withBody(objectMapper.writeValueAsString(
                                        new ApiFindHardshipResponse().withStatus(HardshipReviewStatus.IN_PROGRESS)))));
    ...
    void givenInvalidRequest_whenUpdateHardshipIsInvoked_thenFailsWithBadRequest() throws Exception {
        ApiPerformHardshipRequest request = new ApiPerformHardshipRequest(
                new HardshipReview()
                        .withSolicitorCosts(
                                new SolicitorCosts().withRate(BigDecimal.ONE).withHours(null)),
    ...
    void givenValidRequest_whenCreateHardshipIsInvoked_thenOkResponse() throws Exception {
        ...
        wiremock.stubFor(post(urlEqualTo("/hardship"))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(response))));
    ...
    void givenValidRequest_whenCalculateHardshipForDetailIsInvoked_thenOkResponse() throws Exception {
        ...
        wiremock.stubFor(get(urlEqualTo("/hardship/repId/" + TEST_REP_ID + "/detailType/" + DETAIL_TYPE))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
        ...
        verify(
                exactly(1),
                getRequestedFor(urlEqualTo("/hardship/repId/" + TEST_REP_ID + "/detailType/" + DETAIL_TYPE)));
    }
    ...
    void givenAnEmptyOAuthToken_whenRollbackIsInvoked_thenFailsWithUnauthorizedAccess() throws Exception {
        mvc.perform(MockMvcRequestBuilders.put(ENDPOINT_URL + "/rollback")
                        .content("{}")
                        .contentType(APPLICATION_JSON))
                .andExpect(status().isUnauthorized())
                .andReturn();
    ...
    void givenAValidRequest_whenRollbackHardshipIsInvoked_thenOkResponse() throws Exception {
        wiremock.stubFor(patch(urlEqualTo("/hardship/" + HARDSHIP_ID))
                .willReturn(WireMock.ok().withHeader("Content-Type", String.valueOf(APPLICATION_JSON))));

        mvc.perform(MockMvcRequestBuilders.patch(ENDPOINT_URL_GET)
                        .contentType(APPLICATION_JSON)
                        .content("")
                        .header(HttpHeaders.AUTHORIZATION, BEARER_TOKEN))
                .andExpect(status().isOk());
    ...
    void givenValidRequest_whenCalculateHardshipIsInvoked_thenOkResponse() throws Exception {
        ...
        wiremock.stubFor(get(urlEqualTo(ENDPOINT_URL_FULL_ASSESSMENT_THRESHOLD
                        + TestModelDataBuilder.ASSESSMENT_DATE.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))))
                .willReturn(WireMock.ok()
                        .withHeader("Content-Type", String.valueOf(APPLICATION_JSON))
                        .withBody(objectMapper.writeValueAsString(BigDecimal.TEN))));
