...
class HardshipMapperTest {
    ...
    void givenHardshipReviewDTO_whenFromDtoIsInvoked_thenResponseIsMapped() {
        ...
        softly.assertThat(response.getHardshipReviewId()).isEqualTo(metadata.getHardshipReviewId());

        softly.assertThat(response.getDisposableIncome()).isEqualTo(hardship.getTotalAnnualDisposableIncome());

        softly.assertThat(response.getPostHardshipDisposableIncome())
                .isEqualTo(result.getPostHardshipDisposableIncome());

        softly.assertThat(response.getReviewResult()).isEqualTo(result.getResult());
    }
    ...
    void givenApiPerformHardshipRequest_whenToDtoIsInvoked_thenDtoIsMapped() {
        ApiPerformHardshipRequest hardship = new ApiPerformHardshipRequest()
                .withHardshipMetadata(new HardshipMetadata().withHardshipReviewId(TestModelDataBuilder.HARDSHIP_ID));
        HardshipReviewDTO reviewDTO = new HardshipReviewDTO();

        mapper.toDto(hardship, reviewDTO);

        softly.assertThat(reviewDTO.getHardship()).isEqualTo(hardship.getHardship());
        softly.assertThat(reviewDTO.getHardshipMetadata()).isEqualTo(hardship.getHardshipMetadata());
    }
}
