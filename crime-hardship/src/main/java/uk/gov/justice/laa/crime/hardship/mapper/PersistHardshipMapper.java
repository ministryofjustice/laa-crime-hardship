...
@Component
public class PersistHardshipMapper
        implements RequestMapper<ApiPersistHardshipRequest, HardshipReviewDTO>,
    ...
    public ApiPersistHardshipRequest fromDto(HardshipReviewDTO reviewDTO) {
        ...
        return request.withNworCode(metadata.getReviewReason().getCode())
                .withCmuId(metadata.getCmuId())
                .withReviewResult(isResult ? hardshipResult.getResult() : null)
                .withReviewDate(hardship.getReviewDate())
                .withResultDate(isResult ? LocalDateTime.now() : null)
                .withNotes(metadata.getNotes())
                .withDecisionNotes(metadata.getDecisionNotes())
                .withSolicitorCosts(
                        Objects.isNull(hardship.getSolicitorCosts())
                                ? null
                                : SolicitorCosts.builder()
                                        .rate(hardship.getSolicitorCosts().getRate())
                                        .disbursements(
                                                hardship.getSolicitorCosts().getDisbursements())
                                        .vat(hardship.getSolicitorCosts().getVat())
                                        .estimatedTotal(
                                                hardship.getSolicitorCosts().getEstimatedTotal())
                ...
                .withDisposableIncomeAfterHardship(isResult ? hardshipResult.getPostHardshipDisposableIncome() : null)
                .withReviewDetails(convertHardshipDetails(
                        hardship, metadata.getUserSession().getUserName()));
    }

    private List<ApiHardshipDetail> convertHardshipDetails(HardshipReview hardship, String username) {

        List<ApiHardshipDetail> apiHardshipDetails = Stream.of(
                        hardship.getDeniedIncome(), hardship.getExtraExpenditure())
                ...
                .map(item -> {
                    ...
                    if (item instanceof DeniedIncome deniedIncome) {
                        return detail.withDetailType(HardshipReviewDetailType.INCOME)
                        ...
                        return detail.withDetailType(HardshipReviewDetailType.EXPENDITURE)
                                .withDetailCode(HardshipReviewDetailCode.getFrom(
